...
namespace IF
{
    public class EnemyFireFairyCtrl : EnemyBaseClass
    {
        ...
        IEnumerator StingerShooting(Vector3 directionVector_Normalized)
        {
            foreach (GameObject bulletObj in stingerObjectPool)
            {
                ...
                {
                    bulletObj.transform.SetPositionAndRotation(this.gameObject.transform.position + (gameObject.transform.up * 0.1f), this.gameObject.transform.rotation);
                    yield return null;
        ...
        void CreateBulletObjectPool()
        {
            ...
            {
                GameObject bulletObj = Instantiate(stingerPrefab, gameObject.transform.position + (gameObject.transform.up * 0.1f), gameObject.transform.rotation, GoogleARCore.IF.TowerBuildController.TBController.DefenseStation_Anchor_Tr);
